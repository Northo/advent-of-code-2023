...
ENV VIRTUAL_ENV /opt/advent-of-code-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/advent-of-code/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/advent-of-code/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/advent_of_code/ && touch src/advent_of_code/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/advent-of-code/
