...
def test_is_compatible():
    assert Game(1, [Draw(red=1)]).is_compatible_with(red=1, green=0, blue=0)

    assert not Game(1, [Draw(red=10)]).is_compatible_with(red=1, green=0, blue=0)


def test_min_compatible():
    assert Game(1, [Draw(red=10, green=5), Draw(green=6)]).minimal_compatible_bag() == {
        "red": 10,
        "green": 6,
        "blue": 0,
    }
